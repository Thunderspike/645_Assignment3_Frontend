...
export class SurveyService {
  private surveyURL = 'http://localhost:8080/SurveyMiddleware/api'; // URL to web api
  ...
  getSurvey(id: string): Observable<SurveyDB> {
    ...
    return this.http.get<SurveyDB>(url).pipe(
      ...
      catchError(
        this.handleError<SurveyDB>(`getSurvey id #${id}`, this.emptySurvey)
      )
    );
  }
  ...
  addSurvey(survey: SurveyDB): Observable<SurveyDB> {
    ...
    return this.http
      .post<SurveyDB>(`${this.surveyURL}/survey`, survey, this.httpOptions)
      .pipe(
        ...
        catchError(this.handleError<SurveyDB>('addSurvey', this.emptySurvey))
      );
  }
  ...
  updateSurvey(survey: SurveyDB): Observable<any> {
    ...
    return this.http
      .put(`${this.surveyURL}/survey/${survey.id}`, survey, this.httpOptions)
      .pipe(
        tap((_) =>
          this.log({
            severity: 'success',
        ...
        catchError(this.handleError<any>(`updateSurvey #${survey.id}`, survey))
      );
  }
